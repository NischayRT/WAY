...
export default function WeekDateStrip({ days, selectedDate, onSelectDate, today, minDate }) {
  const dateInputRef = useRef(null);

  const handleCustomDateChange = (e) => {
    const picked = e.target.value;
    if (picked && picked >= minDate && picked <= today) {
      onSelectDate(picked);
    }
  };
          ...
          <button
            ...
            className={`flex flex-col items-center justify-center shrink-0 w-[50px] md:w-full py-2.5 px-1 rounded-xl text-center border transition-all ${
              isDisabled
                ...
                : isSelected
                ? 'border-slate-900 bg-slate-900 text-white shadow-sm dark:border-amber-400 dark:bg-amber-400/10 dark:text-amber-300 dark:shadow-[0_0_15px_rgba(245,158,11,0.25)] active:scale-95'
                : 'border-slate-200/90 dark:border-slate-700/80 bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-300 hover:border-slate-300 dark:hover:border-slate-500 hover:bg-slate-50 dark:hover:bg-slate-800/80 active:scale-95'
            }`}
          >
            <span
